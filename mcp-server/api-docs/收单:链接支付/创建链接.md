# 创建链接 (Create Payment Link)

## Endpoint
```
POST https://pay-gate-uat.payermax.com/aggregate-pay/api/gateway/createPaybylink
```
...
| linkType | string | required | 枚举值：ONETIME，当前仅支持ONETIME | <= 32 characters, Allowed: ONETIME |
| expiresTime | string | optional | 链接有效时长，单位为秒，最长可30天，默认24h，最小1h | <= 10 characters |
| country | string | required | 国家代码，大写字母，如果所传的国家代码与币种不匹配，则以货币代码对应的地区展示收银台。国家上送参见【交易支持国家/地区与币种】 | <= 2 characters |
| currency | string | required | 标价币种，大写字母，参见【交易支持国家/地区与币种】 | <= 3 characters |
| totalAmount | string | required | 标价金额，金额的单位为元。除以下国家外，按照各国币种支持的小数点位上送。注意：巴林、科威特、约旦、阿曼、突尼斯，本币只支持两位小数；印尼、中国台湾、韩国、越南、智利、巴基斯坦、哥伦比亚、日本、冰岛、巴拉圭，本币不支持带小数金额。 | <= 24 characters |
| language | string | optional | 链接页面语言，默认英语 | <= 8 characters |
...
| userInfo | object | required | 用户信息 | - |
| goodsDetails | array[object] | required | 商品信息，支持传多个 | - |
| merchantInfo | object | required | 链接页面展示商户信息 | - |
| notifyUrl | string | required | 服务端回调通知URL，以http/https开头，可以通过MerchantDashboard平台配置商户通知地址，如果订单中上送，则以此为准 | <= 256 characters |
