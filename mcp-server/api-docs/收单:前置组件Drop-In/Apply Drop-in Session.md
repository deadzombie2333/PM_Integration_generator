...
| tokenForFutureUse | boolean | 否 | token支付标识,true标识支持token支付,false表示不支持token支付,默认false | Default: false |
| componentList | array[string] | 否 | 组件列表 | 可选值:CARD, APPLEPAY, GOOGLEPAY |
