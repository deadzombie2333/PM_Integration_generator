# 日终余额查询

## 接口信息

**请求方式:** POST

**请求地址:** `https://pay-gate-uat.payermax.com/aggregate-pay/api/gateway/dayEndBalanceQuery`

**接口说明:** 返回指定日期及时区下的余额信息

## Request

### Headers

| 参数名 | 类型 | 必填 | 说明 | 示例值 |
|--------|------|------|------|--------|
| Content-Type | string | 是 | 内容类型 | application/json |
| sign | string | 是 | 签名信息,请参考技术文档 |  |

### Body (application/json)

| 参数名 | 类型 | 必填 | 说明 | 限制 | 示例值 |
|--------|------|------|------|------|--------|
| version | string | 是 | 接口版本,当前值为:1.3 | - | - |
| keyVersion | string | 是 | 密钥版本,当前值为:1 | <= 8 characters | - |
| requestTime | string | 是 | 请求时间,符合rfc3339规范,格式:yyyy-MM-dd'T'HH:mm:ss.SSSXXX 时间需要在当前时间两分钟内 | <= 32 characters | - |
| appId | string | 是 | 商户应用Id,PayerMax分配给商户应用的唯一标识 | <= 64 characters | - |
| merchantNo | string | 否 | 商户号,商户与PayerMax业务签约时生成的唯一标识 | <= 32 characters | - |
| data | object | 是 | 请求数据体 | - | - |

#### data 对象

| 参数名 | 类型 | 必填 | 说明 | 限制 | 示例值 |
|--------|------|------|------|------|--------|
| accountCurrency | string | 否 | 符合ISO 4217规范. 当未指定币种时,返回全部 | <= 3 characters | IDR |
| accountType | array[string] | 否 | -ACQUIRING 收单账户 -DISBURSEMENT 代发账户可用余额户 当未指定账户类型时,返回全部 | - | - |
| inquiryDate | string | 是 | 支持查询近90天 | <= 32 characters | "2023-01-01" |
| timezone | string | 是 | 目前只支持(UTC 、CST),不区分大小写 | <= 32 characters | CST |

## Response

### 200 成功响应

#### Body (application/json)

| 参数名 | 类型 | 必填 | 说明 | 限制 | 示例值 |
|--------|------|------|------|------|--------|
| code | string | 是 | 返回码,'APPLY_SUCCESS'代表成功 | - | - |
| msg | string | 是 | 返回描述,'Success.' | - | - |
| data | object | 是 | 响应数据体 | - | - |

#### data 对象

| 参数名 | 类型 | 必填 | 说明 | 限制 | 示例值 |
|--------|------|------|------|------|--------|
| timezone | string | 是 | 目前只支持(UTC 、CST) | <= 32 characters | CST |
| acctTime | string | 是 | 账户时间,符合rfc3339规范,格式:yyyy-MM-dd'T'HH:mm:ss.SSSXXX | <= 63 characters | 2022-01-22T10:00:00.500+08:00 |
| accountEndDayTimeTypes | array[object] | 是 | 查询到的账户类型 | - | - |
| errorCode | string | 是 | 错误码 | <= 32 characters | - |
| errorMsg | string | 是 | 错误信息 | <= 256 characters | - |
