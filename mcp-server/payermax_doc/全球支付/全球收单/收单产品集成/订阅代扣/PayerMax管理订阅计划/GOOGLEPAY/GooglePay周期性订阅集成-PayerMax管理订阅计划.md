# GooglePay周期性订阅集成-PayerMax管理订阅计划

该文档介绍了PayerMax管理周期性订阅计划时，周期性订阅集成GooglePay的相关集成步骤，具体包含：创建订阅计划、激活订阅计划以及接收订阅扣款结果等。

## 1. 前端交互

**注意**：收银台模式下，PayerMax管理订阅计划时，订阅计划详情会在PayerMax收银台上展示；但由于GooglePay的限制，GooglePay只会对欧洲经济区（European Economic Area，简称"EEA"）展示订阅计划，非EEA地区只展示付款按钮。
...
- API模式激活订阅计划时，商户需要将Google的支付要素信息上送给PayerMax，否则无法完成订阅计划的激活。
...
```json
{
  ...
  "data": {
    ...
    "paymentDetail": {
      "paymentMethodType": "GOOGLEPAY", //激活时，必传，值为GOOGLEPAY
...
商户需要在商户的收银台上，从Apple钱包中获取Google的支付要素，调用PayerMax激活订阅计划时，需要将Google支付要素解密后，上送给PayerMax完成订阅激活。
...
// 监听点击googlepay按钮的事件，用户选择完订阅计划后，点击googlepayv按钮的时候可以通过这个事件来监听
