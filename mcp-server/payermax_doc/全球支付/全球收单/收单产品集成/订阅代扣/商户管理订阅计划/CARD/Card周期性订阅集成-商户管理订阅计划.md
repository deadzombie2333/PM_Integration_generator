# Card周期性订阅集成-商户管理订阅计划

该文档介绍了商户管理周期性订阅计划时，周期性订阅集成卡支付的相关集成步骤，具体包含：绑定支付方式、获取绑定支付方式结果、发起代扣、获取代扣结果等。
...
```json
{
  ...
  "data": {
    ...
    "paymentDetail": {
      ...
      "mitType": "SCHEDULED", // 必传，MIT类型，周期性代扣时为SCHEDULED
...
商户需要在商户的收银台上，从Apple获取支付要素，调用PayerMax绑定支付方式时，需要将支付要素解密后，上送给PayerMax完成绑定。
...
```json
{
  ...
  "data": {
    "totalAmount": "10", // 代扣金额
    "mitType": "SCHEDULED", // 必填，值为SCHEDULED或UNSCHEDULED
    "currency": "USD", // 必填
    "country": "SA", // 非必填
