# ApplePay周期性订阅集成-商户管理订阅计划

该文档介绍了商户管理周期性订阅计划时，周期性订阅集成ApplePay的相关集成步骤，具体包含：绑定支付方式、获取绑定支付方式结果、发起代扣、获取代扣结果等。

## 1. 前端交互

注意：收银台模式下，商户管理订阅计划时，订阅计划详情会在ApplePay弹层上展示，但不会在PayerMax收银台上展示。
